...
test_that("tif_corpus_validate", {
	...
	tc <- data.frame(doc_id='1', text='foobar', stringsAsFactors=F)
	expect_false(tif_corpus_validate(as.matrix(tc)))
	expect_warning(
		tif_corpus_validate(as.matrix(tc), warn=T),
		"corpus object must inherit the data.frame class"
	)
	expect_false(tif_corpus_validate(unclass(tc)))
	expect_warning(
		tif_corpus_validate(unclass(tc), warn=T),
		"corpus object must inherit the data.frame class"
	)

	# Corpora with only one column
	tc <- data.frame(doc_id='1', stringsAsFactors=F)
	expect_false(tif_corpus_validate(tc))
	expect_warning(
		tif_corpus_validate(tc, warn=T),
